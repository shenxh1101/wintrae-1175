...
from .merger import (
    ...
    preview_merge, format_merge_preview
)
...
def cmd_scan(args):
    ...
    missing_info = [
        ...
        if not f.interviewee or not f.date
    ]
    if missing_info:
        print("【待确认素材】")
        for f in missing_info:
            print(f"    - {f.filename}")
        print()
        print("提示: 使用 report --export-confirm 生成待确认清单，编辑后用 --import-confirm 导入")
...
def cmd_rename(args):
    ...
    confirm_mgr = ConfirmationManager(directory)
    ...
    if from_plan:
        ...
        if os.path.exists(confirm_mgr.confirm_file) and confirm_mgr.load():
            result = confirm_mgr.apply_confirmations(result)
...
def cmd_merge(args):
    ...
    if group_by:
        ...
        results = merge_by_group(
            ...
            confirmation_manager=confirm_mgr
        )
...
def cmd_report(args):
    ...
    if export_confirm:
        entries = confirm_mgr.generate(result)
        human_path = os.path.join(directory, '待确认清单_可读版.txt')
        confirm_mgr.export_human_readable(human_path)
        print(f"待确认清单已生成: {confirm_mgr.confirm_file}")
        print(f"可读版已导出: {human_path}")
        print(f"共 {len(entries)} 条待确认")
        return 0
    ...
    reporter.confirmation_manager = confirm_mgr
